...
def predict_plate(sess,loaded_model,input_dir):
    cnt_plate= len(os.listdir(input_dir))
   
    if cnt_plate==0:
        print("No input plate found")
        return '_'
    else:
        tiger_test = TextImageGenerator(input_dir, 'test', 128, 64, cnt_plate, 4)
        tiger_test.build_data()
        net_inp = loaded_model.get_layer(name='the_input').input
        net_out = loaded_model.get_layer(name='softmax').output
        #print(net_inp,tiger_test)
        for inp_value, _ in tiger_test.next_batch(K):    
            X_data = inp_value['the_input']           
            net_out_value = sess.run(net_out, feed_dict={net_inp:X_data})
            print("net_out_value shape:",np.shape(net_out_value))
           
            pred_texts = decode_batch(net_out_value)
            
            deletefiles(input_dir)
            return pred_texts    
